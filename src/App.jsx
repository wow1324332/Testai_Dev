...
// === 강력한 Firebase Null 에러 방어막 ===
const getSafeId = (idVal) => {
  if (idVal === null || idVal === undefined || idVal === '') return 'temp_fallback_id';
  return String(idVal);
};

const getPublicCollection = (colName) => 
  isCanvas ? collection(db, 'artifacts', canvasAppId, 'public', 'data', getSafeId(colName)) : collection(db, getSafeId(colName));
  
const getPublicDoc = (colName, docId) => 
  isCanvas ? doc(db, 'artifacts', canvasAppId, 'public', 'data', getSafeId(colName), getSafeId(docId)) : doc(db, getSafeId(colName), getSafeId(docId));
...
const App = () => {
  ...
  // === 무한 렌더링 루프 원천 차단 (useMemo를 통한 메모리 참조 고정) ===
  const safeAppCategories = useMemo(() => Array.isArray(appCategories) && appCategories.length > 0 ? appCategories : ['미분류', '홈화면', '결제', '등록', '커뮤니티'], [appCategories]);
  const safeAppComponents = useMemo(() => Array.isArray(appComponents) ? appComponents.filter(Boolean) : [], [appComponents]);
  const objectTypes = useMemo(() => safeAppComponents.length > 0 ? safeAppComponents.map(c => c?.type || 'Unknown') : ['Button', 'Input', 'Select', 'Checkbox', 'Toggle', 'Tab', 'Text', 'Image', 'Icon', 'Container'], [safeAppComponents]);
  const safeDepartments = useMemo(() => Array.isArray(departments) ? departments.filter(Boolean) : [], [departments]);
  ...
  const safeActiveUsers = useMemo(() => Array.isArray(activeUsers) ? activeUsers.filter(Boolean) : [], [activeUsers]);
  ...
  useEffect(() => {
    ...
    const presenceRef = getPublicDoc('presence', user.uid || 'guest');
    const updatePresence = async () => {
      try {
        await setDoc(presenceRef, { 
          lastSeen: Date.now(), uid: user.uid || 'guest', email: manualInfo.memberId || "Guest",
    ...
    if (appState !== 'intro' && appState !== 'login') {
      ...
      }, (err) => console.warn('Snapshot Error (presence):', err));
  ...
  useEffect(() => {
    ...
    const unsubCat = onSnapshot(catRef, (docSnap) => {
      if (docSnap.exists() && Array.isArray(docSnap.data().list)) setAppCategories(docSnap.data().list);
      else setDoc(catRef, { list: ['미분류', '홈화면', '결제', '등록', '커뮤니티'] }).catch(e => console.warn(e));
    ...
    const unsubComp = onSnapshot(compRef, (docSnap) => {
      if (docSnap.exists() && Array.isArray(docSnap.data().list)) setAppComponents(docSnap.data().list);
    ...
    }, (err) => console.warn('Snapshot Error (components):', err));
    ...
    const unsubDept = onSnapshot(deptRef, (docSnap) => {
      if (docSnap.exists() && Array.isArray(docSnap.data().list)) {
         setDepartments([...docSnap.data().list].sort((a, b) => (a?.order || 0) - (b?.order || 0)));
      }
      else setDoc(deptRef, { list: [{ id: 'default', name: '공통 사업부', order: 0 }] }).catch(e => console.warn(e));
    }, (err) => console.warn('Snapshot Error (departments):', err));
  ...
  useEffect(() => {
    ...
    const unsubProjects = onSnapshot(getPublicCollection('projects'), (snapshot) => {
      ...
      setProjects([...projList].sort((a, b) => (b?.order || b?.createdAt || 0) - (a?.order || a?.createdAt || 0)));
    }, (err) => console.warn('Snapshot Error (projects):', err));
    
    // 에러 콜백 추가
    const unsubHistory = onSnapshot(getPublicCollection('history'), (snapshot) => {
      const histList = [];
      snapshot.forEach(doc => histList.push({ id: doc.id, ...doc.data() }));
      setHistory([...histList].sort((a, b) => (b?.order || b?.createdAt || 0) - (a?.order || a?.createdAt || 0)));
    }, (err) => console.warn('Snapshot Error (history):', err));
    return () => { unsubProjects(); unsubHistory(); };
  }, [isAuthReady, user, appState]);

  // === 문제 해결 핵심: 에러 콜백 방어막으로 렌더링 파괴 방지 ===
  useEffect(() => {
    ...
    }, (err) => console.warn('Snapshot Error (currentProject):', err));
    return () => unsubProject();
  ...
  useEffect(() => {
    ...
    if (!isMetaFocused && activeScan) {
      setScanMetaForm(prev => {
         const newTitle = activeScan.title || '';
         const newCategory = activeScan.category || (safeAppCategories.length > 0 ? safeAppCategories[0] : '미분류');
         
         if (prev.title === newTitle && prev.category === newCategory) {
            return prev;
         }
         return { title: newTitle, category: newCategory };
      });
    }
  }, [currentProjectData?.activeScan?.title, currentProjectData?.activeScan?.category, isMetaFocused, safeAppCategories]);

  // Admin Pending Members Listener
  useEffect(() => {
    if (isAdminModalOpen && isAuthReady && user) {
      const unsub = onSnapshot(getPublicCollection('members'), (snapshot) => {
        const pending = [];
        snapshot.forEach(doc => {
          if (doc.data().status === 'PENDING') pending.push({ id: doc.id, ...doc.data() });
        });
        setPendingMembers([...pending].sort((a, b) => (b?.createdAt || 0) - (a?.createdAt || 0)));
      }, (err) => console.warn('Snapshot Error (members):', err)); // 에러 콜백 추가
      return () => unsub();
    }
  }, [isAdminModalOpen, isAuthReady, user]);
  ...
  const saveJiraKey = async (objId, tcId, issueIndex, keyVal) => {
    ...
    const testObjects = Array.isArray(currentProjectData?.activeScan?.testObjects) ? currentProjectData.activeScan.testObjects : [];
    const updatedObjects = testObjects.map(obj => {
      if (obj.id === objId) {
        const updatedCases = (Array.isArray(obj.testCases) ? obj.testCases : []).map(tc => {
          if (tc.id === tcId) {
            const newJiraKeys = [...(Array.isArray(tc.jiraKeys) ? tc.jiraKeys : [])];
  ...
  const handleAdminAuth = () => {
    if (adminPasswordInput === 'djslzja1324!') {
        setIsAdminAuthModalOpen(false);
        setAdminPasswordInput('');
        setAdminAuthErrorMsg('');
        setIsAdminModalOpen(true);
    } else {
        setAdminAuthErrorMsg('비밀번호가 일치하지 않습니다.');
    }
  };

  const approveMember = async (id) => {
      await updateDoc(getPublicDoc('members', id), { status: 'APPROVED' });
  };

  const rejectMember = async (id) => {
      await deleteDoc(getPublicDoc('members', id));
  };

  // --- Issue Handlers (Main) ---
  const updateTestCaseIssue = async (objId, tcId, issueIndex, newValue) => {
    if (!currentProjectId || isProjectClosed) return;
    const testObjects = Array.isArray(currentProjectData?.activeScan?.testObjects) ? currentProjectData.activeScan.testObjects : [];
    const updatedObjects = testObjects.map(obj => {
      if (obj.id === objId) {
        const updatedCases = (Array.isArray(obj.testCases) ? obj.testCases : []).map(tc => {
          if (tc.id === tcId) {
            const newIssues = [...(Array.isArray(tc.issues) ? tc.issues : [''])];
    ...
    const updatedObjects = testObjects.map(obj => {
      if (obj.id === objId) {
        const updatedCases = (Array.isArray(obj.testCases) ? obj.testCases : []).map(tc => {
          if (tc.id === tcId) {
            const newIssues = [...(Array.isArray(tc.issues) ? tc.issues : ['']), ''];
            const newJiraKeys = [...(Array.isArray(tc.jiraKeys) ? tc.jiraKeys : []), null];
  ...
  const removeTestCaseIssue = async (objId, tcId, issueIndex) => {
    ...
    const updatedObjects = testObjects.map(obj => {
      if (obj.id === objId) {
        const updatedCases = (Array.isArray(obj.testCases) ? obj.testCases : []).map(tc => {
          if (tc.id === tcId) {
            const newIssues = (Array.isArray(tc.issues) ? tc.issues : ['']).filter((_, idx) => idx !== issueIndex);
            const newJiraKeys = (Array.isArray(tc.jiraKeys) ? tc.jiraKeys : []).filter((_, idx) => idx !== issueIndex);
  ...
  const handleCustomRegister = async () => {
    ...
    try {
      ...
      await setDoc(memberRef, { id: safeId, password: memberPw, displayName: memberName, photoURL: '', createdAt: Date.now(), status: 'PENDING' });
      
      setAuthMode('login');
      setMemberPw('');
      setLoginErrorMsg("계정 등록이 완료되었습니다. 관리자 승인 후 로그인 가능합니다.");
  ...
  const handleCustomLogin = async () => {
    ...
    try {
      ...
      if (snap.data().status === 'PENDING') { 
         setLoginErrorMsg("관리자 승인이 필요한 계정입니다."); 
  ...
  const addDepartment = async () => {
    ...
    const newDept = { id: `dept_${Date.now()}`, name: newDeptName.trim(), order: safeDepartments.length };
    await updateDoc(getPublicDoc('settings', 'departments'), { list: [...safeDepartments, newDept] });
    setNewDeptName(''); setAdminErrorMsg('');
  };
  const deleteDepartment = async (id) => {
    if (safeDepartments.length <= 1) { setAdminErrorMsg('최소 1개의 사업부는 유지해야 합니다.'); return; }
    const updatedList = safeDepartments.filter(d => d.id !== id).map((d, i) => ({ ...d, order: i }));
    await updateDoc(getPublicDoc('settings', 'departments'), { list: updatedList });
    setAdminErrorMsg('');
  };
  ...
  const handleScanButtonClick = () => {
    if (isAnalyzing) {
       ...
       if (currentProjectId) updateDoc(getPublicDoc('projects', currentProjectId), { isAnalyzing: false });
    } else {
  ...
  const analyzeScreenshot = async (base64Data, imageSrc) => {
    ...
    const callApi = async (attempt = 0) => {
      try {
        ...
        const componentMap = safeAppComponents.reduce((acc, curr) => { acc[curr.type] = curr.cases; return acc; }, {});
    ...
    try {
      ...
      if(currentProjectId) await updateDoc(projectRef, { 'activeScan.testObjects': results, isAnalyzing: false });
    } catch (err) {
      if (err.name === 'AbortError') {
         if(currentProjectId) await updateDoc(projectRef, { isAnalyzing: false });
      } else {
         if(currentProjectId) await updateDoc(projectRef, { isAnalyzing: false, 'activeScan.title': '⚠️ 분석 실패', 'activeScan.description': '원인: ' + err.message });
      }
    } finally {
      setScanAbortController(null);
    }
    ...
    if (file && file.type.startsWith('image/')) {
      ...
      reader.onload = async (event) => {
        ...
        const newScan = currentProjectData?.activeScan ? {
          ...
          title: '', description: '', category: safeAppCategories.length > 0 ? safeAppCategories[0] : '미분류',
  ...
  const saveMetaFields = async () => {
    ...
    await updateDoc(getPublicDoc('projects', currentProjectId), { 'activeScan.title': scanMetaForm?.title || '', 'activeScan.category': scanMetaForm?.category || '미분류' });
  };
  ...
  const toggleTestCaseStatus = async (objId, tcId, newStatus) => {
    ...
    const updatedObjects = testObjects.map(obj => {
      if (obj.id === objId) {
         const updatedCases = (Array.isArray(obj.testCases) ? obj.testCases : []).map(tc => tc.id === tcId ? { ...tc, status: newStatus } : tc);
  ...
  const saveEditedObject = async () => {
    if (!currentProjectId || isProjectClosed || !editObjForm?.label?.trim()) return;
    const componentMap = safeAppComponents.reduce((acc, curr) => { acc[curr.type] = curr.cases; return acc; }, {});
    const testObjects = Array.isArray(currentProjectData?.activeScan?.testObjects) ? currentProjectData.activeScan.testObjects : [];
    const updatedObjects = testObjects.map(obj => {
      if (obj.id === editingObjId) {
         let newTestCases = Array.isArray(obj.testCases) ? obj.testCases : [];
  ...
  const addNewObject = async () => {
    if (!currentProjectId || isProjectClosed || !addObjForm?.label?.trim()) return;
    const componentMap = safeAppComponents.reduce((acc, curr) => { acc[curr.type] = curr.cases; return acc; }, {});
  ...
  const addTestCaseToObject = async (objId) => {
    if (!currentProjectId || isProjectClosed || !newTestCaseLabel.trim()) return;
    const testObjects = Array.isArray(currentProjectData?.activeScan?.testObjects) ? currentProjectData.activeScan.testObjects : [];
    const updatedObjects = testObjects.map(obj => {
      if (obj.id === objId) {
        const newCase = { id: `tc_${Date.now()}`, label: newTestCaseLabel.trim(), status: 'PENDING' };
        const updatedCases = [...(Array.isArray(obj.testCases) ? obj.testCases : []), newCase];
        const isFail = updatedCases.some(tc => tc.status === 'FAIL');
        const isPending = updatedCases.some(tc => tc.status === 'PENDING');
        let parentStatus = 'PENDING';
        if (isFail) parentStatus = 'FAIL'; else if (!isPending && updatedCases.length > 0) parentStatus = 'PASS';
  ...
  const saveEditedTestCase = async (objId) => {
    if (!currentProjectId || isProjectClosed || !editingTestCaseLabel.trim()) return;
    const testObjects = Array.isArray(currentProjectData?.activeScan?.testObjects) ? currentProjectData.activeScan.testObjects : [];
    const updatedObjects = testObjects.map(obj => {
      if (obj.id === objId) {
        const updatedCases = (Array.isArray(obj.testCases) ? obj.testCases : []).map(tc => tc.id === editingTestCaseId ? { ...tc, label: editingTestCaseLabel.trim() } : tc);
        return { ...obj, testCases: updatedCases };
      }
      return obj;
    });
  ...
  const deleteTestCaseFromObject = async (objId, tcId) => {
    ...
    const updatedObjects = testObjects.map(obj => {
      if (obj.id === objId) {
        const updatedCases = (Array.isArray(obj.testCases) ? obj.testCases : []).filter(tc => tc.id !== tcId);
        const isFail = updatedCases.some(tc => tc.status === 'FAIL');
        const isPending = updatedCases.some(tc => tc.status === 'PENDING');
        let parentStatus = 'PENDING';
        if (isFail) parentStatus = 'FAIL'; else if (!isPending && updatedCases.length > 0) parentStatus = 'PASS';
  ...
  const saveCurrentToHistory = async () => {
    ...
    if (!currentProjectId || !scan || !scan.image || isProjectClosed) return;
    setIsSavingReport(true);
    try {
      const testObjects = Array.isArray(scan.testObjects) ? scan.testObjects : [];
      const stats = {
      ...
      await addDoc(getPublicCollection('history'), {
        ...
        category: scanMetaForm?.category || scan?.category || (safeAppCategories.length > 0 ? safeAppCategories[0] : '미분류'), image: scan.image, base64Image: scan.base64Image,
        testObjects: [...testObjects], stats, savedBy: manualInfo.memberId || user?.uid || 'anonymous', savedByName: manualInfo.displayName || "GUEST", savedByPhoto: manualInfo.photoURL || null
      });
  ...
  const updateHistoryTestCaseIssue = (objId, tcId, issueIndex, newValue) => {
    ...
    const updatedObjects = (Array.isArray(selectedHistoryItem.testObjects) ? selectedHistoryItem.testObjects : []).map(obj => {
      if (obj.id === objId) {
         const updatedCases = (Array.isArray(obj.testCases) ? obj.testCases : []).map(tc => {
           if (tc.id === tcId) {
             const newIssues = [...(Array.isArray(tc.issues) ? tc.issues : [''])];
  ...
  const handleHistoryDrop = async (e, targetId) => {
    ...
    const groupedHistoryTemp = safeHistory.filter(h => h?.projectId === currentProjectId).reduce((acc, item) => { const cat = item?.category || '미분류'; if(!acc[cat]) acc[cat]=[]; acc[cat].push(item); return acc; }, {});
    const folderHistories = groupedHistoryTemp[selectedFolder] || [];
  ...
  const saveHistoryChanges = async () => {
     if (!selectedHistoryItem || isProjectClosed) return;
     await updateDoc(getPublicDoc('history', selectedHistoryItem.id), { testObjects: Array.isArray(selectedHistoryItem.testObjects) ? selectedHistoryItem.testObjects : [], stats: selectedHistoryItem.stats });
  ...
  const renameGroup = async () => {
    ...
    const newList = safeAppCategories.map(c => c === editGroupOldName ? editGroupNewName.trim() : c);
    await updateDoc(getPublicDoc('settings', 'categories'), { list: [...new Set(newList)] });
    const items = safeHistory.filter(h => h?.projectId === currentProjectId && h?.category === editGroupOldName);
  ...
  const executeDeleteGroup = async () => {
     const items = safeHistory.filter(h => h?.projectId === currentProjectId && h?.category === deleteGroupTarget);
     for (const item of items) await deleteDoc(getPublicDoc('history', item.id));
     await updateDoc(getPublicDoc('settings', 'categories'), { list: safeAppCategories.filter(c => c !== deleteGroupTarget) });
  ...
  const saveCategory = async () => {
    ...
    await updateDoc(getPublicDoc('settings', 'categories'), { list: [...safeAppCategories, newCatName.trim()] });
    setNewCatName('');
  };
  const deleteCategory = async (catToRemove) => {
    if (catToRemove === '미분류') return;
    await updateDoc(getPublicDoc('settings', 'categories'), { list: safeAppCategories.filter(c => c !== catToRemove) });
  };
  const addComponentType = async () => {
    if(!newCompType.trim() || !user) return;
    await updateDoc(getPublicDoc('settings', 'components'), { list: [...safeAppComponents, { type: newCompType.trim(), cases: [] }] });
    setNewCompType('');
  };
  const deleteComponentType = async (type) => {
    await updateDoc(getPublicDoc('settings', 'components'), { list: safeAppComponents.filter(c => c?.type !== type) });
  };
  const addTestCaseToComp = async (type) => {
    if(!newTestCase.trim() || !user) return;
    const newList = safeAppComponents.map(c => c?.type === type ? { ...c, cases: [...(Array.isArray(c.cases) ? c.cases : []), newTestCase.trim()] } : c);
    await updateDoc(getPublicDoc('settings', 'components'), { list: newList });
    setNewTestCase('');
  };
  const deleteTestCaseFromComp = async (type, caseIndex) => {
    const newList = safeAppComponents.map(c => c?.type === type ? { ...c, cases: (Array.isArray(c.cases) ? c.cases : []).filter((_, idx) => idx !== caseIndex) } : c);
    await updateDoc(getPublicDoc('settings', 'components'), { list: newList });
  };

  const projectHistory = safeHistory.filter(h => h?.projectId === currentProjectId);
  ...
  const displayGroups = Object.keys(groupedHistory).sort((a, b) => {
     let idxA = safeAppCategories.indexOf(a); let idxB = safeAppCategories.indexOf(b);
     return (idxA === -1 ? 999 : idxA) - (idxB === -1 ? 999 : idxB);
  });

  const activeScan = currentProjectData?.activeScan || null;
  const isAnalyzing = currentProjectData?.isAnalyzing || false;
  const safeTestObjects = Array.isArray(activeScan?.testObjects) ? activeScan.testObjects.filter(Boolean) : [];
      ...
      <div className="min-h-screen bg-[#001529] flex flex-col items-center justify-center p-6 text-white font-sans">
        ...
        <div className="relative mb-10">
          <div className="relative z-10 flex items-center justify-center mb-6">
            <img src="/icon.png" alt="App Icon" className="w-40 h-40 object-contain" onError={(e) => { e.currentTarget.style.display = 'none'; e.currentTarget.parentElement.querySelector('svg').classList.remove('hidden'); }} />
      ...
      <div className="min-h-screen bg-[#001529] flex flex-col items-center justify-center p-6 text-white overflow-hidden font-sans relative">
        ...
        {isAdminAuthModalOpen && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-[#001224]/80 backdrop-blur-md animate-in fade-in duration-300">
            <div className="bg-gradient-to-b from-[#001529] to-[#000b14] border border-white/10 w-full max-w-xs rounded-[2rem] shadow-[0_0_40px_rgba(0,102,255,0.15)] p-6 flex flex-col text-white relative overflow-hidden animate-in zoom-in-95 duration-300">
              <div className="flex items-center justify-between mb-6 border-b pb-4 border-white/10">
                <div>
                  <h2 className="text-xl font-black uppercase tracking-tight text-white italic">Admin <span className="text-[#0066FF] not-italic">Login</span></h2>
                  <p className="text-[9px] font-bold text-blue-300/50 uppercase tracking-widest mt-1">관리자 비밀번호 입력</p>
                </div>
                <button onClick={() => { setIsAdminAuthModalOpen(false); setAdminPasswordInput(''); setAdminAuthErrorMsg(''); }} className="p-2 hover:bg-white/10 rounded-full text-white/40 hover:text-white transition-all"><X className="w-5 h-5" /></button>
              </div>
              <input type="password" placeholder="Password" className="w-full px-5 py-3.5 bg-white/5 border border-white/10 rounded-xl outline-none focus:border-[#0066FF] text-white text-sm font-bold transition-all mb-4" value={adminPasswordInput} onChange={(e) => setAdminPasswordInput(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleAdminAuth()} autoFocus />
              {adminAuthErrorMsg && <p className="text-rose-400 text-center text-xs font-bold mb-4">{adminAuthErrorMsg}</p>}
              <button onClick={handleAdminAuth} className="w-full py-3 bg-[#0066FF] text-white rounded-xl font-black text-sm shadow hover:bg-blue-500 transition-colors">접속</button>
            </div>
          </div>
        )}
            ...
            <div className="bg-gradient-to-b from-[#001529] to-[#000b14] border border-white/10 w-full max-w-sm rounded-[2rem] shadow-[0_0_40px_rgba(0,102,255,0.15)] p-6 flex flex-col text-white relative overflow-hidden animate-in zoom-in-95 duration-300">
              <div className="flex items-center justify-between mb-6 border-b pb-4 border-white/10">
                <div>
                  ...
                  <p className="text-[9px] font-bold text-blue-300/50 uppercase tracking-widest mt-1">서버 및 계정 관리</p>
                </div>
              ...
              <div className="flex bg-white/5 rounded-xl p-1 mb-6 border border-white/10">
                <button onClick={() => setAdminTab('dept')} className={`flex-1 py-2.5 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${adminTab === 'dept' ? 'bg-[#0066FF] text-white shadow-md' : 'text-white/40 hover:text-white/70'}`}>사업부 관리</button>
                <button onClick={() => setAdminTab('members')} className={`flex-1 py-2.5 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${adminTab === 'members' ? 'bg-[#0066FF] text-white shadow-md' : 'text-white/40 hover:text-white/70'} flex items-center justify-center gap-1.5`}>
                   가입 대기 {pendingMembers.length > 0 && <span className="w-4 h-4 bg-rose-500 text-white rounded-full flex items-center justify-center text-[8px] shadow-sm">{pendingMembers.length}</span>}
                </button>
              </div>
              ...
              {adminTab === 'dept' && (
                <>
                  ...
                  <div className="space-y-2 overflow-y-auto max-h-60 pr-1 scrollbar-hide">
              ...
              {adminTab === 'members' && (
                 <div className="space-y-2 overflow-y-auto max-h-60 pr-1 scrollbar-hide">
                    {pendingMembers.length === 0 ? (
                       <p className="text-xs text-white/40 text-center py-8 font-bold">승인 대기 중인 계정이 없습니다.</p>
                    ) : (
                       pendingMembers.map(member => (
                          <div key={member.id} className="flex flex-col gap-3 p-4 bg-white/5 border border-white/10 rounded-xl">
                             <div className="flex items-center justify-between">
                                <span className="text-sm font-black text-white/90 truncate">{member.displayName}</span>
                                <span className="text-[10px] text-white/40 font-bold bg-white/5 px-2 py-1 rounded-md">{member.id}</span>
                             </div>
                             <div className="flex gap-2">
                                <button onClick={() => approveMember(member.id)} className="flex-1 py-2 bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 rounded-lg text-[10px] font-black uppercase hover:bg-emerald-500 hover:text-white transition-all shadow-sm">승인 허가</button>
                                <button onClick={() => rejectMember(member.id)} className="flex-1 py-2 bg-rose-500/20 text-rose-400 border border-rose-500/30 rounded-lg text-[10px] font-black uppercase hover:bg-rose-500 hover:text-white transition-all shadow-sm">거절/삭제</button>
                             </div>
                          </div>
                       ))
                    )}
      ...
      <div className="min-h-screen bg-[#001224] text-white flex flex-col relative overflow-hidden font-sans">
        ...
        <div className="relative z-10 flex flex-col min-h-screen p-8 md:p-16 max-w-[1400px] mx-auto w-full">
            ...
            <div className="flex items-center gap-4">
              <div className="relative">
                <button onClick={() => setIsUserMenuOpen(!isUserMenuOpen)} style={{ animation: 'heartbeatGlow 2s ease-in-out infinite' }} className="w-9 h-9 rounded-full overflow-hidden border border-white/20 hover:border-[#0066FF] transition-all shrink-0 bg-slate-800 flex items-center justify-center">
                  {currentUserPhoto ? <img src={currentUserPhoto} alt="" className="w-full h-full object-cover" /> : <span className="font-bold text-xs text-white">{String(currentUserDisplayName || 'U')[0]}</span>}
                </button>
                {isUserMenuOpen && (
              ...
              <div className="flex items-center gap-2.5 bg-gradient-to-r from-white/10 to-transparent backdrop-blur-md border border-white/20 px-4 py-2 rounded-full shadow-[0_0_15px_rgba(0,102,255,0.2)]">
                ...
                <span className="text-xs font-black tracking-widest uppercase text-white/90">{safeActiveUsers.length} Online</span>
              </div>
            </div>
          </header>
          ...
          <main className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 md:gap-6">
            ...
            {safeProjects.filter(p => p && (p.departmentId || 'default') === (currentDepartment?.id || 'default')).map(proj => {
              const projectMembers = safeActiveUsers.filter(u => u && u.currentProjectId === proj.id);
              return (
                  ...
                  <div className="flex flex-col h-full justify-between min-h-[130px]">
                    ...
                    <div className="mt-4 pt-4 border-t border-white/5 flex items-center justify-between">
                      <div className="flex items-center gap-2.5">
                        <div className="w-6 h-6 rounded-full overflow-hidden border border-white/20">{proj.createdByPhoto ? <img src={proj.createdByPhoto} alt="" className="w-full h-full object-cover" /> : <div className="bg-slate-700 w-full h-full flex items-center justify-center text-[10px] font-bold">{String(proj.createdByName || 'U')[0]}</div>}</div>
                        <span className="text-xs font-bold text-white/40">{proj.createdByName || "익명"}</span>
                      </div>
                      <div className="flex -space-x-2">
                        {projectMembers.slice(0, 3).map((m, idx) => (
                          <div key={idx} className="w-6 h-6 rounded-full bg-slate-800 border border-[#001224] flex items-center justify-center text-[8px] font-black overflow-hidden shadow-xl">{m.photoURL ? <img src={m.photoURL} alt="" className="w-full h-full object-cover" /> : String(m.displayName || 'U')[0]}</div>
                        ))}
                      </div>
                    </div>
                  </div>
  ...
  return (
    <div className={`h-screen w-full bg-[#F8FAFC] text-slate-900 font-sans flex overflow-hidden ${isAuthorizedEntering ? 'anim-cinematic-enter' : ''}`}>
          ...
          <div className="bg-white w-full max-w-lg rounded-[2rem] shadow-2xl p-6 flex flex-col max-h-[85vh]">
            ...
            {!editingCompType ? (
                  ...
                  {safeAppComponents.map(comp => (
                    <div key={comp.type} className="flex items-center justify-between px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl hover:border-blue-200 transition-colors cursor-pointer group" onClick={() => setEditingCompType(comp.type)}>
                      <div>
                         ...
                         <p className="text-[10px] text-slate-400 font-bold mt-0.5">{(Array.isArray(comp.cases) ? comp.cases : []).length}개의 기본 케이스</p>
                      </div>
              ...
              <div className="flex flex-col flex-1 overflow-hidden">
                ...
                <div className="space-y-2 overflow-y-auto flex-1 pr-1">
                   {(() => {
                      const currentComp = safeAppComponents.find(c => c?.type === editingCompType);
                      const currentCases = Array.isArray(currentComp?.cases) ? currentComp.cases : [];
                      if (currentCases.length === 0) return <p className="text-xs text-slate-400 font-bold italic text-center py-6">등록된 기본 케이스가 없습니다.</p>;
                      return currentCases.map((tc, idx) => (
                       ...
                       >
                          ...
                          <div className="flex-1 min-w-0 w-full text-center py-2">
                             ...
                             <div className="flex justify-center gap-1.5">
                                <span className="text-[9px] font-black px-2 py-0.5 bg-emerald-50 text-emerald-600 rounded-md border border-emerald-100 uppercase tracking-tighter">{item?.stats?.pass || 0} Pass</span>
                                <span className="text-[9px] font-black px-2 py-0.5 bg-rose-50 text-rose-600 rounded-md border border-rose-100 uppercase tracking-tighter">{item?.stats?.fail || 0} Fail</span>
                             </div>
                          </div>
                       </div>
                    ))}
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-8 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
           <div className="relative w-full max-w-5xl bg-white rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-300">
                 ...
                 <div>
                    ...
                    <div className="flex items-center gap-2 mt-2">
                       <div className="w-5 h-5 rounded-full overflow-hidden border border-slate-200">
                          {selectedHistoryItem.savedByPhoto ? <img src={selectedHistoryItem.savedByPhoto} className="w-full h-full object-cover" alt="" /> : <div className="w-full h-full bg-slate-200 flex items-center justify-center text-[10px] font-bold">{String(selectedHistoryItem.savedByName || 'U')[0]}</div>}
                       </div>
                       ...
                       <span className="text-[10px] font-bold text-slate-500">{selectedHistoryItem.createdAt ? new Date(selectedHistoryItem.createdAt).toLocaleString() : ''}</span>
                    </div>
                 </div>
              ...
              <div className="flex-1 overflow-y-auto scrollbar-hide p-8 grid grid-cols-1 lg:grid-cols-2 gap-8 bg-slate-50">
                 ...
                 <div className="space-y-4">
                    {(Array.isArray(selectedHistoryItem.testObjects) ? selectedHistoryItem.testObjects : []).map(obj => {
                       const safeHistoryCases = Array.isArray(obj?.testCases) ? obj.testCases.filter(Boolean) : [];
                       return (
                       <div key={obj.id} className="p-4 border rounded-2xl flex flex-col bg-white shadow-sm hover:shadow-md transition-shadow">
                          ...
                          {safeHistoryCases.length > 0 && (
                             <div className="mt-3 pt-3 border-t border-slate-100 space-y-1">
                                {safeHistoryCases.map(tc => {
                                   const safeHistoryIssues = Array.isArray(tc?.issues) ? tc.issues : [''];
                                   const safeHistoryJira = Array.isArray(tc?.jiraKeys) ? tc.jiraKeys : [];
                                   return (
                                        ...
                                        <div className="w-full mt-1 pt-3 border-t border-rose-100/50 flex flex-col gap-2">
                                           <div className="flex items-center justify-between cursor-pointer group/issueToggle" onClick={() => toggleIssueFolder(tc.id)}>
                                              <span className="text-[10px] font-black text-rose-500 uppercase tracking-widest flex items-center gap-1 hover:text-rose-600 transition-colors">
                                                 ...
                                                 <AlertOctagon className="w-3 h-3" /> Issues ({safeHistoryIssues.length})
                                              </span>
                                           ...
                                           {!collapsedIssueTcIds.includes(tc.id) && (
                                              <div className="flex flex-col gap-1.5 mt-1 animate-in fade-in slide-in-from-top-1 duration-200">
                                                 {safeHistoryIssues.map((issue, idx) => (
                                                    <div key={idx} className="flex items-start flex-col gap-1 mt-1">
                                                       <div className="flex items-center gap-2 w-full">
                                                          ...
                                                          <button onClick={() => handleCopySingleIssue(String(issue || ''))} className="p-1.5 bg-white border border-slate-200 text-slate-400 hover:text-blue-500 rounded-lg hover:bg-blue-50 transition-colors shadow-sm shrink-0" title="이슈 복사"><Copy className="w-3.5 h-3.5" /></button>
                                                          {!isProjectClosed && (
                                                          ...
                                                          {currentProjectData?.jiraIntegration && currentProjectData?.jiraEpicUrl && !isProjectClosed && (
                                                             safeHistoryJira[idx] ? (
                                                                <button onClick={() => {
                                                                   ...
                                                                   if (domain) window.open(`${domain}/browse/${safeHistoryJira[idx]}`, '_blank');
                                                                   ...
                                                                   {safeHistoryJira[idx]} <ExternalLink className="w-3 h-3" />
                                                                </button>
                                                                ...
                                                                <button onClick={() => {
                                                                   setJiraCreateModal({ objId: obj.id, tcId: tc.id, issueIndex: idx, summary: String(issue || ''), description: '' });
      ...
      {/* --- Main Workspace (Authorized Editor UI) --- */}
      <div 
        ref={mainScrollRef}
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative">
                <button 
                  ...
                  title="사용자 정보"
                >
                  {currentUserPhoto ? <img src={currentUserPhoto} className="w-full h-full object-cover" alt="" /> : <span className="text-blue-600 font-black text-sm">{String(currentUserDisplayName || 'U')[0]}</span>}
                </button>
              ...
              <div className="bg-white rounded-[2.5rem] shadow-2xl shadow-slate-200 border border-slate-200 overflow-hidden flex-1 flex flex-col min-h-[450px]">
                <div className="p-6 border-b flex items-center justify-between bg-slate-50/50">
                  ...
                  {!isProjectClosed && activeScan?.image && (
                    ...
                    >
                       {isAnalyzing ? 'STOP SCAN' : (safeTestObjects.length > 0 ? 'RESCAN' : 'SCAN')}
                    </button>
                  )}
                </div>
                  ...
                  {isAnalyzing ? (
                    ...
                    <div className="space-y-4 pb-12">
                      ...
                      {safeTestObjects.map(obj => {
                        ...
                        const safeCases = Array.isArray(obj?.testCases) ? obj.testCases.filter(Boolean) : [];
                          ...
                          <div key={obj.id} className={`p-5 rounded-[1.5rem] border transition-all flex flex-col group ${obj.status === 'PASS' ? 'bg-emerald-50 border-emerald-100' : obj.status === 'FAIL' ? 'bg-rose-50 border-rose-100' : 'bg-white border-slate-100 shadow-sm hover:shadow-md hover:border-blue-200'}`}>
                             ...
                             <div className="flex items-center justify-between cursor-pointer w-full" onClick={() => setExpandedObjId(prev => prev === obj.id ? null : obj.id)}>
                                ...
                                <div className="flex items-center gap-2 shrink-0" onClick={e => e.stopPropagation()}>
                                   ...
                                   <button disabled={isProjectClosed || safeCases.length > 0} onClick={() => { 
                                      if (safeCases.length > 0) {
                                         setExpandedObjId(prev => prev === obj.id ? null : obj.id);
                                      } else {
                                         toggleStatus(obj.id, obj.status); 
                                      }
                                   }} className={`w-24 py-2.5 rounded-xl font-black text-[10px] uppercase shadow-lg transition-all ml-1 ${(isProjectClosed || safeCases.length > 0) ? 'cursor-pointer' : 'active:scale-90'} ${obj.status === 'PASS' ? 'bg-emerald-500 text-white' : obj.status === 'FAIL' ? 'bg-rose-500 text-white' : 'bg-slate-900 text-white'}`}>
                                      {obj.status}
                                   </button>
                             ...
                             {expandedObjId === obj.id && (
                                <div className="w-full mt-4 pt-4 border-t border-slate-200/60 space-y-2 cursor-default" onClick={e => e.stopPropagation()}>
                                   {safeCases.length > 0 ? safeCases.map(tc => {
                                      ...
                                      const safeIssues = Array.isArray(tc?.issues) ? tc.issues : [''];
                                      const safeJiraKeys = Array.isArray(tc?.jiraKeys) ? tc.jiraKeys : [];
                                         ...
                                         {tc.status === 'FAIL' && (
                                           <div className="w-full mt-1 pt-3 border-t border-rose-100/50 flex flex-col gap-2">
                                              <div className="flex items-center justify-between cursor-pointer group/issueToggle" onClick={() => toggleIssueFolder(tc.id)}>
                                                 <span className="text-[10px] font-black text-rose-500 uppercase tracking-widest flex items-center gap-1 hover:text-rose-600 transition-colors">
                                                    {!collapsedIssueTcIds.includes(tc.id) ? <ChevronDown className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}
                                                    <AlertOctagon className="w-3 h-3" /> Issues ({safeIssues.length})
                                                 </span>
                                                 <div className="flex gap-1.5" onClick={e => e.stopPropagation()}>
                                                    {!isProjectClosed && (
                                                             ...
                                                             {currentProjectData?.jiraIntegration && currentProjectData?.jiraEpicUrl && !isProjectClosed && (
                                                                safeJiraKeys[idx] ? (
                                                                   <button onClick={() => {
                                                                      let domain = '';
                                                                      try { domain = new URL(currentProjectData.jiraEpicUrl).origin; } catch(e) {}
                                                                      if (domain) window.open(`${domain}/browse/${safeJiraKeys[idx]}`, '_blank');
                                                                   }} className="px-3 py-1.5 bg-rose-50 text-rose-600 border border-rose-200 rounded-lg text-[10px] font-black uppercase hover:bg-rose-100 hover:text-rose-700 transition-colors shadow-sm flex items-center gap-1.5 shrink-0" title="Jira 이동">
                                                                      {safeJiraKeys[idx]} <ExternalLink className="w-3 h-3" />
                                                                   </button>
                                                                ) : jiraLinkingTarget === `${obj.id}_${tc.id}_${idx}` ? (
                                                                   <input 
                                                                      type="text" 
